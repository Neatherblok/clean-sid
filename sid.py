...
def signed_distance(feats1, feats2, FLAGS_dict, num_subsets=20, max_subset_size=100):
    ...
    ### in case I need it...
    n = feats1.shape[1]
    Unif_batch_size = m = 100  # min(min(feats1.shape[0], feats2.shape[0]), max_subset_size)
...
def get_folder_features(fdir, model=None, num_workers=12, num=None,
                        shuffle=False, seed=0, batch_size=128,
...
if __name__ == '__main__':
    ...
    plot_path = run_loc + '/sid_data_' + FLAGS_dict['data'] + '_noise_' + FLAGS_dict['noise'] + '_' + FLAGS_dict[
        'model'] + '_order_' + str(FLAGS_dict['order'])
    ...
    if FLAGS_dict['FID_flag']:
        fid_val = fid.compute_fid(FLAGS_dict['fakes_dir'], FLAGS_dict['reals_dir'], mode=FLAGS_dict['metric_mode'],
                                  device=device, batch_size=FLAGS_dict['batch_size'])
        f = open(res_file, 'a')
        f.write("\n FID :" + str(fid_val) + '\n')
        f.close()
        print('\n FID :' + str(fid_val) + '\n')
    if FLAGS_dict['KID_flag']:
        kid_val = fid.compute_kid(FLAGS_dict['fakes_dir'], FLAGS_dict['reals_dir'], mode=FLAGS_dict['metric_mode'],
                                  device=device, batch_size=FLAGS_dict['batch_size'])
        f = open(res_file, 'a')
        f.write("\n KID :" + str(kid_val) + '\n')
        f.close()
